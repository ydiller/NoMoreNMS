...
class SetTransformer(nn.Module):
    ...
    def forward(self, X):
        if self.mode == 'image':
            input_shape = X.shape #(m_batchsize, channels, width , height)
            X = X.view(input_shape[0], -1, input_shape[2] * input_shape[3])
        elif self.mode == 'dense':
            X = torch.unsqueeze(X, dim=1)
        else:
            assert False, "Transformer mode is not recognized"

        out = self.dec(self.enc(X)).squeeze()
        # out = out.view(-1)
        # if out.shape[0] < self.embed_dim:
        #     p = torch.zeros(self.embed_dim - out.shape[0]).to('cuda')
        #     out = torch.cat([out, p])
        # else:
